...
def auth():
    ...
    # time.
    if os.path.exists('/Users/nitinchoudhary/budgeteer/files/token.pickle'):
        with open('/Users/nitinchoudhary/budgeteer/files/token.pickle', 'rb') as token:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(
                '/Users/nitinchoudhary/budgeteer/files/credentials.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open('/Users/nitinchoudhary/budgeteer/files/token.pickle', 'wb') as token:
